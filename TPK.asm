...
color		word	15
msg	byte	"You have now entered the Kernel", 13, 10
	byte	"Please enter a key to run 32 tasks!!!", 13, 10
	byte	"-----------------------------------------------", 0

.code
jmp	main

; Function: Prints a spinner on the screen
; Receives: None
; Returns:  None
; Requires: None
; Clobbers: bx
...
; Function: Prints '.' accross the screen
; Receives: None
; Returns:  None
; Requires: None
; Clobbers: ax, bx, dx
...
init_stack1 proc
	...
	mov	sp, [sp_array + si]
	; push flags and cs for future iret and task1 address for "yield's" return
	pushf
	push	cs
	push	offset task1
	; push regs
	...
	pushw	0
	pushw	0
	push	bx		; location param for task
	pushw	0
	mov	[sp_array + si], sp
	push	bp
	pop	sp
...
main_loopy2:
	...
	mov	dx, 1193180 / 1000	; something much faster :)
